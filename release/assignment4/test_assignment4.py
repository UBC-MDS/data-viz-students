...
def test_1_4(answer):
    assert not answer is None, "Your answer does not exist. Have you passed in the correct variable?"
    sha1(str(answer.lower() + 'm').encode('utf8')).hexdigest() == 'a67acd6e68f872b807a1c9afe3a2453564e57877', "Your answer is incorrect. Please try again. What information does a density plot provide?"
    return(success)
...
def test_1_6(answer):
    assert not answer is None, "Your answer does not exist. Have you passed in the correct variable?"
    assert answer.spec.mark == 'bar', "Make sure you are creating a bar plot using the 'mark_bar()' function."
...
def test_1_7(answer):
    assert not answer is None, "Your answer does not exist. Have you passed in the correct variable?"
    sha1(answer.lower().encode('utf8')).hexdigest() == "fd1286353570c5703799ba76999323b7c7447b06", "Your answer is incorrect, please try again."
    return(success)

def test_1_8(answer):
    assert not answer is None, "Your answer does not exist. Have you passed in the correct variable?"
    sha1(str(answer.lower() + 'l').encode('utf8')).hexdigest() == 'ef3ecccf258fa062c5c6521a4887d40541963af7', "Your answer is incorrect. Please try again. What is the shape of the curve? Which way is it leaning?"
    return(success)
...
def test_2_3(answer):
    ...
    assert answer.spec.mark.size == 10, "Make sure you are setting the szie of the points to 10."
    assert answer.spec.encoding.x.field.repeat == 'column', "Make sure you are passing the columns to the repeat argument."
    assert answer.spec.encoding.y.field.repeat == 'row', "Make sure you are passing the rows to the repeat argument."
...
def test_4_7(answer):
    assert not answer is None, "Your answer does not exist. Have you passed in the correct variable?"
    sha1(answer.lower().encode('utf8')).hexdigest() == "63b12989474b64a1b1f8c2035d6ef2a4640845c6", "Your answer is incorrect, please try again."
    return(success)

def test_4_8(answer):
    assert not answer is None, "Your answer does not exist. Have you passed in the correct variable?"
    sha1(re.sub('[.!@#$]', '', answer).lower().encode('utf8')).hexdigest() == "64ad577207c3fe433812973e9fddefc16292d8f2", "Your answer is incorrect, please try again. Which studio has the largest square in the action column?"
    return(success)
