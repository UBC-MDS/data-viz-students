...
def test_1_1(answer):
    ...
    assert answer.encoding.y.scale.zero == False, "Make sure you are using the 'alt.Scale(zero=False)' for the x axis."
    ...
    assert str(answer.title) == 'Undefined', "Make sure your plot does not have a main title."
    return(success)
...
def test_3_1(answer):
    ...
    assert round(min(answer.age),2) == 16, "The min age is incorrect. Are you mering correctly"
    return(success)
...
def test_3_5(answer):
    ...
    assert answer2.encoding.y.sort == ['Thai', 'Hungarian', 'Cantonese', 'Basque','German', 'Turkish', 'Italian', 'Catalan',
                                       ...
                                       'French'], "Make sure that you are plotting the mean information rate in acending order." 
...
def test_4_1(answer):
    ...
    assert answer.url == "https://cdn.jsdelivr.net/npm/vega-datasets@v1.29.0/data/world-110m.json", "Make sure you are using the correct URL."
    return(success)
...
def test_4_3(answer):
    ...
    assert sha1(str(c2.transform[0]['from']['fields']).encode('utf8')).hexdigest() == "cff45fb0181de1738867a8d7acbb65480bcdc542", "Make sure you are using the correct fields for the look up transformation"
...
def test_5_2(answer):
    ...
    assert not answer.title.text.islower(), "Make sure the plot title is capitalized."
    assert answer.title.text.count("_") == 0, "Make sure your plot title does not contain underscores"
    assert not answer.title.subtitle[0] is None, "Make sure you are providng a subtitle for your plot"
